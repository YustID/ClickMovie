    ...
    <div class="flex justify-between lg:justify-around py-5 px-5 bg-[#2B2A2F] fixed top-0 z-[999] right-0 left-0">
        ...
        <div class="hidden bg-[#2B2A2F] fixed z-0 right-0 left-0 mt-5" id="myLinks">
          <ul>
            ...
            <a href="#home"><li class="ml-2 py-2 text-white hover:bg-orange-500">Tv List</li></a>
          </ul>
        </div>
        {{-- End Hamburger --}}
      ...
      <div class=" flex items-center justify-center mt-32">
        ...
        <p class="absolute bg-black px-4 text-xl font-bold text-white">Tv List</p>
      </div>

      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
        @foreach($tvlist as $i)
          <div class="mt-20 text-center">
              <a href="{{ 'genre/' . $i['id'] . '/tvlist' }}" class="border-2 rounded-full border-solid border-white text-lg px-5 py-1 text-white hover:border-red-400 hover:text-black hover:bg-white">{{ $i['name'] }}</a>
          </div>
        @endforeach
      </div>
